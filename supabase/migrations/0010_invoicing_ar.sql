-- Applied live on 2026-07-03 as "invoicing_ar".
-- Invoicing / accounts receivable on loads (one invoice per load):
-- invoice_number, invoiced_at, invoice_due_date, paid_at, all surfaced in
-- loads_enriched for the Billing page.

alter table public.loads add column if not exists invoice_number text;
alter table public.loads add column if not exists invoiced_at timestamptz;
alter table public.loads add column if not exists invoice_due_date date;
alter table public.loads add column if not exists paid_at timestamptz;
